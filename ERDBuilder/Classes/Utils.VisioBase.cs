...
namespace LinkeD365.ERDBuilder
{
    public static partial class Utils
    {
        ...
        private static XDocument GetXMLFromPart(PackagePart packagePart)
        {
            ...
            return XDocument.Load(partStream, LoadOptions.PreserveWhitespace); ;
        }
...
XDocument partXML)
   ...
   XDocument packagePart, string elementType)
    ...
    string attributeName, string attributeValue)
        ...
        private static void CreateNewPage(string name, int bpfCount)
        {
            // Need to check first to see whether the part exists already.
            var packageLocation = new Uri(Uri.EscapeUriString($"/visio/pages/flowPage{bpfCount}.xml"), UriKind.Relative);

            if (!templatePackage.PartExists(packageLocation))
            {
                // Create a new blank package part at the specified URI 
                // of the specified content type.
                PackagePart newPackagePart = templatePackage.CreatePart(packageLocation,
                    templatePage.ContentType);
            ...
            var rel = templatePages.CreateRelationship(packageLocation,
                TargetMode.Internal,
                "http://schemas.microsoft.com/visio/2010/relationships/page");

            var pagesXml = GetXMLFromPart(templatePages);
            var xDocPage = pagesXml.Root.Elements().First(ele => ele.Attribute("NameU").Value == "Page-1");
            var pageXML = new XElement(xDocPage);
            ...
            pageXML.SetAttributeValue("ID", xDocPage.Parent.Elements().Count());
            ...
            xDocPage.Parent.Add(new XElement(pageXML));
            SaveXDocumentToPart(templatePages, pagesXml);

        }

        private static void RemoveTemplate()
        {
            templatePackage.DeletePart(templatePage.Uri);

            var pagesXML = GetXMLFromPart(templatePages);
            pagesXML.Root.Elements().First(ele => ele.Attribute("NameU").Value == "Page-1").Remove();


            Uri pagesURI = templatePages.Uri;
            string contentType = templatePages.ContentType;

            var relationships = templatePages.GetRelationships().Where(rel => rel.Id != "rId1");
            templatePackage.DeletePart(templatePages.Uri);
            PackagePart newPages = templatePackage.CreatePart(pagesURI, contentType);
